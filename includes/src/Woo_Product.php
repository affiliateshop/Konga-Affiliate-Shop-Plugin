...
class Woo_Product {
	...
	public static function add_meta_data( $product_id, $args, $category ) {
		...
		wp_set_object_terms( $product_id, 'konga', 'product_tag' );

		// update post visibility
		update_post_meta( $product_id, '_visibility', 'visible' );

		self::set_feature_image( $args['image_url'], $product_id );

		if ( 'yoast_seo' == kg_seo_plugin() ) {
			self::wordpress_seo_yoast( $product_id, $args['title'], $args['description'] );
		} elseif ( 'seo_ultimate' == kg_seo_plugin() ) {
			self::seo_ultimate( $product_id, $args['title'], $args['description'], $args['image_url'] );
		}
